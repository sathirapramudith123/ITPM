...
// Helper function to notify employer (called from jobController)
export const notifyEmployer = async (jobId, type) => {
  try {
    const job = await Job.findById(jobId);
    if (!job) return;

    const message = type === 'job_application' 
...
// Helper function to notify job seekers about matching jobs (called from jobController)
